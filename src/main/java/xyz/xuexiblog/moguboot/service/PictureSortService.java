package xyz.xuexiblog.moguboot.service;

import xyz.xuexiblog.moguboot.entity.PictureSort;
import java.util.List;

/**
 * 图片分类表(PictureSort)表服务接口
 *
 * @author makejava
 * @since 2020-04-21 12:32:25
 */
public interface PictureSortService {

    /**
     * 通过ID查询单条数据
     *
     * @param uid 主键
     * @return 实例对象
     */
    PictureSort queryById(String uid);

    /**
     * 查询多条数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    List<PictureSort> queryAllByLimit(int offset, int limit);

    /**
     * 新增数据
     *
     * @param pictureSort 实例对象
     * @return 实例对象
     */
    PictureSort insert(PictureSort pictureSort);

    /**
     * 修改数据
     *
     * @param pictureSort 实例对象
     * @return 实例对象
     */
    PictureSort update(PictureSort pictureSort);

    /**
     * 通过主键删除数据
     *
     * @param uid 主键
     * @return 是否成功
     */
    boolean deleteById(String uid);

}